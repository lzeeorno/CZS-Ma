...
from dataset.dataset import Dataset, Dataset_ssl_lits2017, Dataset_ssl_lits2017_png
# from dataset.dataset import Dataset,Dataset_ssl_lits2017
from dataset.Dataset_3D import Dataset3D
...
import pandas as pd
from net import (Unet, res_unet_plus, R2Unet, sepnet, KiU_Net,
                 Unet3D, ResUformer, ResUformer_tiny,
                 ResUformer_base, SMAFormer, Uformer, SMAFormerV2, SMAFormerV3, SMAFormer_git,
                 PrecSegNet, PrecSegNet_png, PrecSegNet_V2)
...
#     elif mode_Symbol == -1:
#         pad = mode_size
#         img = img[:,pad:args.img_size - pad, pad:args.img_size - pad]
#         label = label[:, pad:args.img_size - pad, pad:args.img_size - pad]
#
#     # 顺逆旋转随机角度
#     img_list = []
#     label_list = []
...
#     # print('img.shape,label.shape:',img.shape,label.shape)
#     # 按比例还原图片大小
#     factor = (3/img.shape[0], args.img_size / img.shape[1], args.img_size / img.shape[2])
#     img = zoom(img, factor,order=2)
#     factor = (2/label.shape[0], args.img_size / label.shape[1], args.img_size / label.shape[2])
#     label = zoom(label, factor, order=0)
#
#     # img = img[np.newaxis, :, :]
#     # print('img.shape,label.shape:',img.shape,label.shape)
#     print('Data Augumentation = True')
#     return img, label
...
def parse_args():
    ...
    parser.add_argument('--model_name', default='PrecSegNet_png',
                        choices=['Unet', 'AttUnet', 'res_unet_plus', 'R2Unet', 'R2AttU_Net', 'sepnet', 'KiU_Net',
                                 'Unet3D', 'ResUformer_large', 'ResT', 'ResUformer_tiny', 'ResUformer_base',
                                 'SMAFormer','Uformer', 'SMAFormerV2', 'SMAFormerV3',
                                 'PrecSegNet', 'PrecSegNet_png'])
    # pre trained
    ...
    parser.add_argument('--loss', default='newBCEDiceLoss')
    ...
    parser.add_argument('-b', '--batch_size', default=10, type=int,
                        metavar='N', help='SMAFormer_LiTS_npy:12,PrecSegNet_LiTS_png:8')
...
def main():
    ...
    if args.loss == 'BCEWithLogitsLoss':
        ...
        criterion = losses.newBCEDiceLoss().cuda()
    ...
    if args.model_name == 'Unet3D':
        model = Unet3D.UNet3d(in_channels=1, n_classes=2, n_channels=32)
    ...
    if args.model_name == 'res_unet_plus':
        ...
        resUnet_path = '../trained_models/LiTS_res_unet_plus/2023-12-31-20-58-24/epoch21-val_loss:0.0287-val_dice2:0.7574_model.pth'
    ...
    if args.model_name == 'ResUformer' or args.model_name=='ResUformer_large':
        model = ResUformer.ResUformer(args)
        resUformer_path = '../trained_models/LiTS_ResUformer_large/2024-04-28-16-51-21/epoch48-val_loss:0.0100-val_dice2:0.9309_model.pth'
        print('ResUformer Large selected!')
    if args.model_name == 'ResUformer_tiny':
        model = ResUformer_tiny.ResUformer(args)
        print('ResUformer_tiny selected!')
    if args.model_name == 'ResUformer_base':
        model = ResUformer_base.ResUformer(args)
        print('ResUformer_big selected!')
    if args.model_name == 'SMAFormer':
        # model = SMAFormer.SMAFormer(args)
        model = SMAFormer_git.SMAFormer(args)
        print('SMAFormer selected!')
        SMAFormer_pretrained_path = '../trained_models/LiTS_SMAFormer/2024-06-23-15-01-51/epoch21-val_loss:0.0643-val_dice2:0.6760_model.pth'
    if args.model_name == 'SMAFormerV2':
        model = SMAFormerV2.SMAFormer(args)
        print('SMAFormerV2 selected!')
    if args.model_name == 'SMAFormerV3':
        model = SMAFormerV3.SMAFormer(args)
        print('SMAFormerV3 selected!')
        SMAFormerV3_pre_trained_path = '../pre_trained/epoch229-val_loss:0.0491-val_dice2:0.0262_model.pth'
    if args.model_name == 'Uformer':
        model = Uformer.Uformer(args)
        print('Uformer selected!')
    if args.model_name == 'PrecSegNet_png':
        # model = PrecSegNet_png.PrecSegNet()
        model = PrecSegNet_V2.PrecSegNet()
        pretrained_path = '../pre_trained/PrecSegNet_png_epoch140-val_loss:0.0157-val_dice2:94_model.pth'
        print('PrecSegNet_png selected!')
    ...
    for i, epoch in enumerate(range(args.epochs)):
        ...
        #
        # log = log._append(tmp, ignore_index=True)
        # log.to_csv('./trained_models/{}/{}/log_normalDataAug_newDice_newLoss.csv'.format(args.name, timestamp), index=False)
        #
        # trigger += 1
        # val_loss = val_log['loss']
        # if val_loss < best_loss:
        #     torch.save(model.state_dict(), './trained_models/{}/{}/epoch{}-val_loss:{:.4f}-val_dice2:{:.4f}_model.pth'.format(args.name,timestamp,epoch,val_log['loss'],val_log['dice_2']))
        #     best_loss = val_loss
        #     print("=> saved best model")
        #     trigger = 0
        #
